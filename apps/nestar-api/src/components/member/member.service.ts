...
import { Follower, Following, Followings, MeFollowed } from '../../libs/dto/follow/follow';
...
export class MemberService {
	...
	public async login(input: LoginInput): Promise<Member> {
		...
		const response: Member | null = await this.memberModel.findOne({ memberNick }).select('+memberPassword').exec();
	...
	public async getMember(memberId: ObjectId, targetId: ObjectId): Promise<Member> {
		...
		if (memberId) {
			//record view 
			const viweInput = { memberId: memberId, viewRefId: targetId, viewGroup: ViewGroup.MEMBER }
